...
> **优先复用 Seafile 官方镜像与现有实现——无论开源还是仅二进制依赖。
> 只有当官方组件不能满足 CE 扩展需求时，才自行适配或替换。是否开源不是首要约束。**

这条与下面的成本模型是一回事：复用一个官方镜像 = 零 fork 成本、零维护面。
所以 Compose **默认直接引用官方镜像**（metadata-server、seasearch、notification、
thumbnail、sdoc、seafile-ai……），不重新打包进 CloudFile 镜像——更新和替换更简单。
