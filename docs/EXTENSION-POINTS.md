...
### 缺口 4：S3 主存储的后端选择是硬编码 🟡

`common/obj-store.c:28` 直接调 `obj_backend_fs_new`，没有分发点。这是 S3 唯一的
新增登记项。**seafobj（Python 读取侧）上游已自带 s3/ceph/swift/alioss 后端**，不需要 fork。
