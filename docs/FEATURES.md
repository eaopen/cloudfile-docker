...
| 69 | ACL 能力门禁 `acl-e2e.yml` | 🟡 | **本轮补齐**。此前只有孤立的 `acl_matrix.py`，没有任何东西调用它——"有测试文件"和"有门禁"在权限系统里差别巨大。顺带给 `acl_matrix.py` 补了 `--insecure`（写于 TLS 改动之前，自签证书下必然连不上）。已在本机跑起来并**当场抓到第 71 项那个缺陷**；矩阵全绿尚未达成 |
...
| 71 | ACL 主体身份解析 | ✅ | 🔴 **矩阵抓到的真实缺陷**。Seafile 14 把身份与邮箱拆开（账号主键是 `<hex>@auth.local`，邮箱降为登录属性），而终判拿到的是身份。**按邮箱下发的规则永远匹配不上，且不报错、不记日志、接口返回 200**——管理员以为限制了访问，实际没有。四个写入路径（库主/管理员 × 增/删）与"有效权限"接口现在都在写入前解析，解析不了直接拒绝。**运行时验证：用身份下规则时 ACL 完全正确**（`restricted` 逐条改写为 `r`、`secret` 从列举消失、`/restricted/sub` 继承 `r`、`/secret` 403），说明强制链路本来就对，错的只是存进去的内容 |
...
1. 🔴 **让 ACL 六入口矩阵全绿**。矩阵已经能跑起来了，第一次运行就抓到第 71 项
   （按邮箱下发的规则永远匹配不上），修复后正在重验。**在矩阵全绿之前 ACL 不能
   合回 `dev`**——这是 [BRANCHES.md](BRANCHES.md) 第六节第 5 条，不因为它是自己
   定的规则就绕过。

   > 这一条的价值已经兑现了：单元测试（C 62 / Python 52）全绿、静态检查全绿、
   > 基线门禁全绿，而那个缺陷**一个都没被拦住**——因为它们证明的是"求解器算得
   > 对"，而缺陷在"存进去的东西根本进不了求解器"。只有把栈起起来、拿另一个用户
   > 的 token 去敲每个入口才会显形。
