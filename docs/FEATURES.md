...
>
> **六入口矩阵 23/23 通过**，开着 `CF_ENABLE_DIR_ACL` 的冒烟 12/12。
...
**已通过自己的 E2E 门禁（23/23）。** 规格、用例集与矩阵随该能力一起进 `dev`，
...
| 69 | ACL 能力门禁 `acl-e2e.yml` | ✅ | **本轮补齐**。此前只有孤立的 `acl_matrix.py`，没有任何东西调用它——"有测试文件"和"有门禁"在权限系统里差别巨大。顺带给 `acl_matrix.py` 补了 `--insecure`（写于 TLS 改动之前，自签证书下必然连不上）。已在本机跑通，**23/23**。第一次运行就当场抓到第 71 项那个缺陷——这条门禁的价值在它存在的第一天就兑现了 |
...
1. 🟠 **验证 WebDAV 读侧补丁**（第 31 项）。补丁已写好并确认能对 pinned ref
   ...
   > 才会显形。
2. 🟠 **在 CI 上完整跑一次三条门禁**（checks / build-and-e2e / acl-e2e）。
   本机已全绿，CI 与本机在架构、磁盘配额、PATH 上都有已知差异，而十二次构建
   失败里有六次正是栽在这类边界。
