...
| 目录级 ACL | 已完成 | 本项目新增 Hub/C 实现；复用 CE 库权限和组 | 前端、后端、数据结构 | Pro 平替 | `cf_dir_acl`、Seafile 身份/组 | [`acl-semantics.md`](acl-semantics.md)、[`acl-cases.json`](acl-cases.json)、[`verify-local.sh cap acl`](../tools/verify-local.sh) | 不适合 PR | 目标接近细粒度目录权限；当前证据不构成与 Pro 完全兼容。同步入口每次读取 C 权威结果，启用后故障 fail closed；六入口（REST/上传下载/打包下载/同步/WebDAV/移动重命名）加即时撤权与管理员清空容器矩阵 37/37 通过。**2026-08-17 更新**：MANAGE 并入 admin（管理维度独立，acl-semantics.md §7）、同类型合并取最高（否决仍生效）、文件规则 > 目录规则、默认回落 CE——规格、`acl-cases.json` v2 与 C/Python 实现均已同步（Python 78 用例、C 67 检查全绿）；**目录级委托管理（V2）已实现**：`cf_dir_admin` 表 + `can_manage` 单一判定 + `dir-admin`/`admin dir-admin` API；分享/成员端点仍按库级 `is_repo_admin`。**V3.1 已落地（2026-08-18）**：`PermissionService` 门面（`effective_perm` + `can_manage`）作为权限判定单一入口，钩子链路与 351 个 `check_folder_permission` 调用点零改动，等价性测试冻结行为。 |
