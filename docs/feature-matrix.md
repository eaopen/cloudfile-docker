...
| 目录级 ACL | 验证中 | 本项目新增 Hub/C 实现；复用 CE 库权限和组 | 前端、后端、数据结构 | Pro 平替 | `cf_dir_acl`、Seafile 身份/组 | [`acl-semantics.md`](acl-semantics.md)、[`acl-cases.json`](acl-cases.json)、[`acl-e2e.yml`](../.github/workflows/acl-e2e.yml) | 不适合 PR | 目标接近细粒度目录权限；当前证据不构成与 Pro 完全兼容。已补 active-authority outage/revision 红色契约；在 Hub/C/Go 实现并覆盖 Web、同步和 WebDAV 前不得标为完成。 |
