...
用一个只会说"不"的假 provider，逐入口确认拒绝确实发生且没有产生提交：
...
同一份 `fileop-cases.json` 分别驱动 C 与 Go 的 operation 归类与路径规范化，
两端结论必须逐条相同。WebDAV 不单独验证归类（它经 C），但要验证**拒绝确实
传导到了 WebDAV 客户端**，而不是被吞成 500。
