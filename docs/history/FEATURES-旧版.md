...
> **用途**：记录能力范围、完成度、验证证据和已知限制。
> **适用版本**：CloudFile `14.0.0-cf.0`，基于 Seafile CE 14 源码重构。
> **状态**：当前状态快照；`✅`、`🟡`、`⬜` 的证据口径见正文，发布构建仍以 `release.yaml` 和对应门禁结果为准。
> **边界**：表内明确区分 CE 原生复用、CloudFile 新增机制与官方/第三方外部组件。
...
> 那句话已经作废，大量 🟡 因此转 ✅。
>
> **基线门禁已在本机与 CI 验证**：本机已再次从源码完整复验；CI 的快速检查与
> 构建 + E2E 工作流均成功（`30048595750`、`30048596090`）。仍应在升级依赖、
> 更换架构或调整构建环境后重跑，因为架构、磁盘配额、PATH 都是已知差异面。
...
| 10 | Compose 一键部署 | ✅ | 核心栈及 8 个可选 profile（`worker`、`search`、`metadata`、`ai`、`office`、`convert`、`s3`、`full`）已定义；各能力的实际运行状态以专项门禁为准，`full` 不包含仅属于 `s3` 的 MinIO |
...
| 37 | 操作日志 / 审计 | ✅ | `CF_ENABLE_AUDIT` 提供系统管理员 API 与 UI 清单（按库、操作者、操作、对象类型筛选），默认只列文件/目录。Server 对所有提交发布 `repo-update`；seafevents 对提交树做 diff 后持久化 `Activity`（创建、修改、删除、重命名、移动、恢复）。完整镜像已验 API、筛选、旧路径与管理员页面。 |
