...
> **用途**：记录 SSO、Metadata Server、OnlyOffice 和检索相关的上游源码探针与可复现结论。
> **适用版本**：CloudFile `14.0.0-cf.0`，对照 Seafile CE 14 源码及 `release.yaml` 锁定组件。
> **状态**：证据记录；探针 0、1、3 已完成，OnlyOffice 探针 2 尚未完成。
> **边界**：本文判断 CE 可复用机制与 CloudFile 必须新增部分；外部镜像是否可取得、许可证和服务承诺需独立核验。
...
> [history/修订史与教训.md](history/修订史与教训.md)。
