...
> **用途**：定义预览路由、文件锁、OnlyOffice、签入签出与本地专业软件编辑的目标架构。
> **适用版本**：CloudFile `14.0.0-cf.0`，基于 Seafile CE 14 源码重构。
> **状态**：实施中；统一写入生命周期和部分 CE 控制面已实现，生产写回、完整锁服务及端到端验收仍以正文状态表为准。
> **边界**：CE 提供原生预览与部分 OnlyOffice 接口；CloudFile 新增锁、票据和写回终判；OnlyOffice、SeaDoc、桌面 Agent 与浏览器扩展是独立组件。
