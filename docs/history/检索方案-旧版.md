...
> **用途**：定义 SeaSearch 默认路径、Meilisearch 可替换路径、索引契约和 ACL 组合限制。
> **适用版本**：CloudFile `14.0.0-cf.0`，基于 Seafile CE 14 源码重构。
> **状态**：P0/P1/P2 已实现但尚无文档记录的真实容器成功跑次；单元、配置和静态检查已覆盖。
> **边界**：CE/seafevents 复用官方 SeaSearch 链路；CloudFile 新增精确解门、provider 和 Meilisearch worker；搜索引擎镜像、许可证及容量属于外部组件。
